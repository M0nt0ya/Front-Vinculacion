...
export class AuthService {
  ...
  signUp(payload: SignUpPayload) {
    return this.http.post("http://localhost:8083/auth/sign-up", payload);
  }

  signIn(payload: SignInRequestPayload) {
    const self = this;
    return this.http.post<SignInResponsePayload>("http://localhost:8083/auth/sign-in", payload).pipe<SignInResponsePayload>(
      map(response=>{
  ...
  logout() {
    const self = this;
    return this.http.post("http://localhost:8083/auth/logout", {username: this.getUsername(), refreshToken: this.localStorage.retrieve("refreshToken")}).subscribe({complete(){
  ...
  refreshAccessToken(): Observable<SignInResponsePayload> {
    return this.http.post<SignInResponsePayload>("http://localhost:8083/auth/refresh-token", {
  ...
  findAllUsernames(): Observable<String[]> {
    return this.http.get<String[]>("http://localhost:8083/auth/usernames");
  }
}
